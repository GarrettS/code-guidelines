...
If the task involves JS, HTML, or CSS, read `code-guidelines.md` and `code-philosophy.md` before writing or reviewing code. Apply them as session constraints.

## Before every commit

1. Run `bash bin/pre-commit-check.sh` — catches mechanical violations.
2. Review the diff against the Patterns and Fail-Safe sections of `code-guidelines.md`.
