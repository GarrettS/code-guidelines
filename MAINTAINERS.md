...
Built templates (`CLAUDE.example.md`, `CODEX.example.md`) are generated by `tools/build-contracts.sh` from `adapters/shared-base/AGENT.md` + per-agent overlays. The build is plain concatenation (`cat`). Do not put maintainer comments in the shared base contract or overlay files — they will appear verbatim in emitted project contracts. See `adapters/shared-base/README.md` for the shared source layout.
...
├── adapters/
│   ├── shared-base/            # adapter-neutral contract source and shared generation docs
│   │   ├── AGENT.md
│   │   └── README.md
