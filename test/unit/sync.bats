...
Body for ${skill_name}"
  ...
  run bash -lc "cd \"$CASE_DIR\" && bash tools/build-adapter-skills.sh >/dev/null"
  [ "$status" -eq 0 ]
...
@test "generated Claude skill keeps YAML frontmatter first" {
  first_line="$(head -1 "$TARGET")"
  [ "$first_line" = "---" ]
}

@test "generated Claude skill has DO NOT EDIT header after frontmatter" {
  run awk 'BEGIN{delim=0; header=0} {if ($0 == "---") delim++; if (index($0, "DO NOT EDIT") > 0 && delim == 2) header=1} END{exit header ? 0 : 1}' "$TARGET"
  [ "$status" -eq 0 ]
}

@test "generated Claude skill has no DO NOT EDIT header before frontmatter" {
